...
def process_emt_pipeline_auto(
    ...
    original_name = os.path.splitext(os.path.basename(src_emt))[0]
    ...
    # Yurak urish rampasi monoton bo‘lishi uchun
    if a_rest <= b_post:
        for i in range(1, N):
    ...
    # ⚙️ Time ustunini raqamga aylantirish (xato holatlarda NaN → 0)
    time_values = pd.to_numeric(
        df[time_col], errors="coerce").fillna(0).astype(float)

    ecg_df = pd.DataFrame({"Time": time_values, "ECG": hr_int})
